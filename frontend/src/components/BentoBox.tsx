...
export default function BentoBox() {
    ...
    const cardClass = "bg-[var(--surface)] p-10 rounded-3xl border border-[var(--line)] shadow-[0_2px_20px_rgb(0,0,0,0.03)] flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl transition-all duration-500 group";

    return (
        <section id="features" className="w-full max-w-6xl mx-auto px-6 py-24 border-t border-[var(--line)]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="text-center mb-16">
                <h2 className="text-3xl md:text-4xl font-bold tracking-[-0.02em] text-[var(--ink)] mb-4">
            ...
            >
                ...
                <motion.div variants={fadeUp} className={`md:col-span-2 ${cardClass} relative overflow-hidden`}>
                    <div className="relative z-10">
                        <h3 className="text-2xl font-medium mb-2 text-[var(--ink)]">Multi-Tenant Workspaces</h3>
                        <p className="text-[var(--ink-2)] max-w-sm">
                            Complete data isolation. Every client&apos;s API keys are AES-256 encrypted and scoped entirely to their dedicated workspace.
                        </p>
                    </div>
                    <motion.div
                        className="absolute -bottom-10 -right-10 w-64 h-64 bg-[var(--page)] rounded-full border border-[var(--line)] flex items-center justify-center"
                        whileHover={{ scale: 1.15, rotate: 10 }}
                        transition={{ type: "spring", stiffness: 200 }}
                    >
                        <div className="w-40 h-40 bg-[var(--surface)] rounded-full shadow-sm border border-[var(--line)] flex items-center justify-center">
                            <svg className="w-12 h-12 text-[var(--ink)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <motion.div variants={fadeUp} className={cardClass}>
                    <div className="relative z-10">
                        <motion.div
                            ...
                            className="w-10 h-10 bg-[var(--page)] rounded-full flex items-center justify-center mb-6 border border-[var(--line)]"
                        >
                            <svg className="w-5 h-5 text-[var(--ink)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
                            </svg>
                        </motion.div>
                        <h3 className="text-xl font-medium mb-2 text-[var(--ink)]">Live Google Analytics Sync</h3>
                ...
                <motion.div variants={fadeUp} className={cardClass}>
                    <div>
                        <motion.div
                            whileHover={{ scale: 1.15 }}
                            className="w-10 h-10 bg-[var(--page)] rounded-full flex items-center justify-center mb-6 border border-[var(--line)] group-hover:bg-[#EEF2FF] dark:hover:bg-[var(--accent)]/12 transition-colors duration-300"
                        >
                            <svg className="w-5 h-5 text-[var(--ink)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        ...
                        <h3 className="text-xl font-medium mb-2 text-[var(--ink)]">Automated Strategy Insights</h3>
                        <p className="text-[var(--ink-2)] text-sm">
                        ...
                        <div className="flex items-center space-x-2 mb-4">
                            {["Google", "Meta", "LinkedIn"].map((platform, i) => (
                                <motion.div
                                    ...
                                    className="px-3 py-1 rounded-full bg-[var(--surface)] border border-[var(--line)] text-xs font-semibold text-[var(--ink-2)] shadow-sm"
                                >
                                    {platform}
                                </motion.div>
                            ))}
                        </div>
                        <h3 className="text-2xl font-medium mb-4 text-[var(--ink)]">White-Labeled Dashboards</h3>
                        <p className="text-[var(--ink-2)] mb-6 max-w-md">
