...
export default function Flo({ size = 80, variant = "full", className = "", tappable = false }: FloProps) {
  ...
  if (variant === "mark") {
    return (
      <div className={`relative inline-block ${className}`} style={{ width: size, height: size }}>
        <svg viewBox="0 0 140 150" width={size} height={size} aria-hidden="true">
          <defs>
  ...
  return (
    <div
      ...
      style={{ width: size, height: size * (150 / 140), perspective: 600 }}
    >
      <motion.div
        className="w-full h-full cursor-pointer"
        onClick={handleTap}
        role={tappable ? "button" : undefined}
        aria-label={tappable ? "Flo, the ArbFlow mascot" : undefined}
        animate={spinAnimate}
        transition={spinTransition}
        style={{ transformStyle: "preserve-3d" }}
      >
        <div className={!reduceMotion ? "flo-bob w-full h-full" : "w-full h-full"}>
